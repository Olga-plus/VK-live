...
export default class Keyboard {
    ...
    init(langCode) {
        ...
        ['spellcheck', false],
        ['autocorrect', 'off']);
        this.conteiner = create('div', 'keyboard', null, main, ['language', langCode]);
    ...
    generateLayout() {
        ...
        this.rowsOrder.forEach((row, i) => {
            const rowElement = create('div', 'keyboard__row', null, this.conteiner, ['row', i + 1]);
            rowElement.style.gridTemplateColumns = `repeat (${row.length}, 1fr)`;
            row.forEach((code) => {
                const keyObj = this.keyBase.find((key) => key.code === code);
                if (keyObj) {
                    const keyButton = new Key(keyObj);
                    this.keyButtons.push(this.keyButton);
